---
name: purescript
description: PureScript language reference — module structure, imports (top-level only, no qualified paths or local imports), records and row polymorphism, ADTs, newtypes, type classes, Effect / Aff, Halogen essentials, FFI patterns, common pitfalls (no Array cons-pattern, Maybe/Either not in Prelude, explicit forall required, Int vs Number). Use whenever editing or generating PureScript code (.purs files), debugging PureScript compile errors, or answering PureScript questions.
---
